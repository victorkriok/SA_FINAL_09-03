...
namespace api.Data
{
    public class AppDbContext
    {
        ...
        public void SalvarLeitura(Leitura dados)
        {
            const string sql = @"
                INSERT INTO Leituras (id_sensor, tipo_leitura, valor, unidade, descricao, data_leitura)
                VALUES (@id_sensor, @tipo_leitura, @valor, @unidade, @descricao, NOW())";

            using var conn = OpenConnection();
            using var cmd = new MySqlCommand(sql, conn);
            cmd.Parameters.AddWithValue("@id_sensor", dados.id_sensor);
            cmd.Parameters.AddWithValue("@tipo_leitura", dados.tipo_leitura);
            cmd.Parameters.AddWithValue("@valor", dados.valor);
            cmd.Parameters.AddWithValue("@unidade", dados.unidade);
            cmd.Parameters.AddWithValue("@descricao", dados.descricao);
            
            int linhasAfetadas = cmd.ExecuteNonQuery();
            Console.WriteLine($"[DB] Linhas inseridas no MySQL: {linhasAfetadas}");
        }

        public void SalvarAlerta(int idSensor, string tipoAlerta, float valorDetectado, float limiteConfigurado, string mensagem)
        {
            const string sql = @"
                INSERT INTO Alerta (id_sensor, tipo_alerta, valor_detectado, limite_configurado, mensagem, data_alerta)
                VALUES (@id_sensor, @tipo_alerta, @valor_detectado, @limite_configurado, @mensagem, NOW())";

            using var conn = OpenConnection();
            using var comando = new MySqlCommand(sql, conn);
            
            comando.Parameters.AddWithValue("@id_sensor", idSensor);
            comando.Parameters.AddWithValue("@tipo_alerta", tipoAlerta);
            comando.Parameters.AddWithValue("@valor_detectado", valorDetectado);
            comando.Parameters.AddWithValue("@limite_configurado", limiteConfigurado);
        ...
        {
            const string sql = "SELECT * FROM Leituras ORDER BY data_leitura DESC LIMIT 50";
            ...
            while (reader.Read())
            {
                lista.Add(new Leitura {
                    id_leitura = reader.GetInt32("id_leitura"),
                    id_sensor = reader.GetInt32("id_sensor"),
                    tipo_leitura = reader.GetString("tipo_leitura"),
                    valor = reader.GetFloat("valor"),
                    unidade = reader.GetString("unidade"),
                    descricao = reader.GetString("descricao"),
                    data_leitura = reader.GetDateTime("data_leitura")
                });
            }
            return lista;
        }

        public List<APIeloT_grupo_4.Models.alerta> ListarAlertas()
        {
            const string sql = "SELECT * FROM Alerta ORDER BY data_alerta DESC LIMIT 50";
            var lista = new List<APIeloT_grupo_4.Models.alerta>();
            ...
            while (reader.Read())
            {
                lista.Add(new APIeloT_grupo_4.Models.alerta {
                    id_alerta          = reader.GetInt32("id_alerta"),
                    id_sensor          = reader.GetInt32("id_sensor"),
                    tipo_alerta        = reader.GetString("tipo_alerta"),
                    valor_detectado    = reader.GetDecimal("valor_detectado"),
                    limite_configurado = reader.GetDecimal("limite_configurado"),
                    mensagem           = reader.GetString("mensagem"),
                    data_alerta        = reader.GetDateTime("data_alerta")
                });
            }
            return lista;
        }
